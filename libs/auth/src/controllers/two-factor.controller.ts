...
export class TwoFactorController {
  ...
  async verifySetup(@Body() body: { setupId: string; token: string }) {
    const result = await this.authService.verify2FASetup(body.setupId, body.token);
  ...
  @Require2FA()
  async disable2FA(@CurrentUser() user: LoggedUser, @Body() body: { token: string }) {
  ...
  @Require2FA()
  async regenerateBackupCodes(@CurrentUser() user: LoggedUser, @Body() body: { token: string }) {
    const result = await this.authService.generateNewBackupCodes(user.id, body.token);
  ...
  async testToken(@Body() body: { secret: string; token: string }) {
    ...
    const verification = this.twoFactorService.verifyToken(body.secret, body.token);
