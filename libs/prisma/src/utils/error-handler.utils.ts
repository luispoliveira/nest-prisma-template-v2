    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      switch (error.code) {
        ...
        case "P2007": // Data validation error
          throw new BadRequestException(`Data validation error: ${error.meta?.database_error}`);
      ...
      throw new InternalServerErrorException("A critical database error occurred");
    } else if (error instanceof Prisma.PrismaClientInitializationError) {
      this.logger.error("Prisma initialization error", error);
      throw new InternalServerErrorException("Database initialization error occurred");
