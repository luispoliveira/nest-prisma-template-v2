...
export class EnhancedHealthController {
  ...
  async checkEnhanced(): Promise<EnhancedHealthCheckResult> {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `Enhanced health check failed after ${duration}ms:`,
        error,
      );
      throw error;
    }
  ...
  @Get('detailed')
  @ApiOperation({
    summary: 'Detailed health report with comprehensive diagnostics',
  })
  ...
  async getDetailedReport() {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `Failed to generate detailed health report after ${duration}ms:`,
        error,
      );
      throw error;
    }
