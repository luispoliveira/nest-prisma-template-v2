...
export const configuration = () => ({
  ...
  healthChecks: {
    database: {
      ...
      timeout: parseInt(process.env.HEALTH_CHECK_DATABASE_TIMEOUT || "5000", 10),
    },
    redis: {
    ...
    memory: {
      ...
      rssThresholdBytes: parseInt(process.env.HEALTH_CHECK_MEMORY_RSS_THRESHOLD || "314572800", 10), // 300MB
    },
    disk: {
      enabled: process.env.HEALTH_CHECK_DISK_ENABLED !== "false",
      path: process.env.HEALTH_CHECK_DISK_PATH || "/",
      thresholdPercent: parseFloat(process.env.HEALTH_CHECK_DISK_THRESHOLD || "0.9"), // 90%
    },
    external: {
      enabled: process.env.HEALTH_CHECK_EXTERNAL_ENABLED !== "false",
      urls: process.env.HEALTH_CHECK_EXTERNAL_URLS?.split(",") || ["https://www.google.com"],
      timeout: parseInt(process.env.HEALTH_CHECK_EXTERNAL_TIMEOUT || "5000", 10),
    },
  },
});
