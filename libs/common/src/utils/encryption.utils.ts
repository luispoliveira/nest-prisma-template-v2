...
export class EncryptionUtils {
  static async encrypt(value: string, password: string): Promise<Buffer> {
    ...
    const encryptedText = Buffer.concat([iv, cipher.update(value), cipher.final()]);
  ...
  static async decrypt(value: Buffer, password: string): Promise<string> {
    ...
    const decryptedText = Buffer.concat([decipher.update(value.subarray(16)), decipher.final()]);
