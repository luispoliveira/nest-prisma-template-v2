...
export class HelpService {
  ...
  async showHelp() {
    ...
    const commands = [
      ...
      },
      {
        ...
        commands: [
          ...
          {
            cmd: 'users:reset-password <id>',
            desc: 'Reset user password by ID',
          },
          {
            cmd: 'users:assign-role <userId> <roleId>',
      ...
      },
      {
        ...
        commands: [
          {
            cmd: 'permissions:create <name> <module>',
            desc: 'Create a new permission',
          },
          { cmd: 'permissions:list', desc: 'List all permissions' },
          {
            cmd: 'permissions:modules',
            desc: 'List all modules with permission counts',
          },
          {
            cmd: 'permissions:activate <id>',
            desc: 'Activate a permission by ID',
          },
          {
            cmd: 'permissions:deactivate <id>',
            desc: 'Deactivate a permission by ID',
          },
          {
            cmd: 'permissions:bulk-create <module>',
            desc: 'Create CRUD permissions for a module',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        commands: [
          {
            cmd: 'db:status',
            desc: 'Check database connection and show statistics',
          },
      ...
      },
      {
        ...
        commands: [
          ...
          {
            cmd: 'queue:clean',
            desc: 'Clean completed and failed jobs from queue',
          },
    ...
    console.log('─'.repeat(50));
    console.log(
      '  yarn command users:create admin@example.com --password=secret123 --role=admin',
    );
    console.log(
      "  yarn command api-keys:create myapp --description='API key for myapp'",
    );
  ...
  async showExamples() {
    ...
    console.log(
      '5. yarn command users:create admin@test.com --role=admin --active',
    );
